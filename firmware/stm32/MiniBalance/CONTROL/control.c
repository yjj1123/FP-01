...
int  inserve_kinematics(float wx,float wy,float wz)
{
	...
	{	
			...
			if(is_right_solution(TargetX,TargetY,TargetZ,theta1,theta2,theta3,theta4)<0)
			  ...
			  Target1   = 500-(radian_to_angle(theta1))*4.44;   //作用到输出  底座
				Target2= 850-4.44*radian_to_angle(theta2);
	      Target3   = 500+(radian_to_angle(theta3))*4.44;
	      Target4   = 500+(radian_to_angle(theta4))*4.44;
...
u16 tempcount[]={0,257,500,491,373,662,514};
u16 tempcount1[]={200,500,600,500,500,600,700};
...
void Action_Six_BUS(u16 *ID_Count1)
{
...
u8 Typecount=0;
void SendCoint(u8 *p,u8 Sum,u8 *redbuff)
{
	char CountSd[10] ={0};
	int i=0;
	CountSd[0] = 0xA5;
					...
					switch(step)	
				{
					...
					case 5:
							    zhua_flag=1;	 
									...
									//	SendCoint(CountBuff,GarbageStype,TempHm);
										Typecount = Typecount +1;
										//printf("type:%d",GarbageStype);
									}
									 // printf("134\n");
